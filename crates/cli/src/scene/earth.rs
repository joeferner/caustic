...
pub fn create_earth_scene(ctx: &RenderContext) -> (Arc<Camera>, Arc<dyn Node>) {
    let image = ctx.image_loader.load_image("assets/earth-map.jpg").unwrap();
